        ...
        <div class="col-md-8">
          <div class="p-4 border rounded bg-light shadow">
            ...
            <form @submit.prevent="createAlbum">
              <div class="mb-3">
                <label for="albumTitle" class="form-label h5">Album Title:</label>
                <input type="text" class="form-control" id="albumTitle" v-model="albumTitle" placeholder="Enter Album Title" required>
              </div>
  ...
  methods: {
    ...
    async createAlbum() {
      try {
        ...
        const response = await axios.post('http://127.0.0.1:5000/api/albums', {
        ...
        this.$router.push('/')
        this.displaySuccessMessage('Album created successfully');
        // Reset form fields
        this.albumTitle = '';
        this.selectedSongs = [];
      } catch (error) {
        this.displayErrorMessage('Error creating album: ' + error.response.data.message);
      }
    },
    async fetchSongs() {
      try {
